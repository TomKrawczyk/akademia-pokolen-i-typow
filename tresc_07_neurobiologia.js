/* ===== AKADEMIA POKOLEN I TYPOW - CZESC 07: MECHANIZM GLEBSZY (SPIN + NEUROBIOLOGIA) ===== */

CONTENT.spin_neuro = `
<img class="mod-icon reveal" src="https://media.base44.com/images/public/69cd0d788375531a373c98b1/99116576f_generated_image.png" alt="" loading="lazy">
<span class="eyebrow">Moduł 25 · Mechanizm głębszy</span>
<h2>SPIN Selling + neurobiologia: biochemiczna mapa rozmowy</h2>
<p class="lede">Wszystko, co widziałeś w częściach II–VI, mówiło Ci CO dopasować do kogo. Ten moduł pokazuje MECHANIZM, dlaczego to w ogóle działa — sekwencję pytań Neila Rackhama (SPIN Selling, badanie na 35 000 rozmów handlowych, 1988) i to, jak w praktyce prowadzi ona klienta przez kolejne stany emocjonalne, aż do decyzji podjętej bez poczucia presji.</p>

<div class="warn"><b>Zastrzeżenie na start:</b> Mapowanie na konkretne neuroprzekaźniki (kortyzol, dopamina, oksytocyna) to <b>uproszczony model treningowy</b> — mnemonic ułatwiający zapamiętanie kolejności i logiki pytań, nie wynik badania obrazowego mózgu klienta. Trzymaj się kolejności i zasad, nie traktuj poziomów hormonów jako dosłownego pomiaru.</p></div>

<h3>Cztery fazy pytań SPIN</h3>
<div class="tiles">
  <div class="tile"><div class="big">S</div><div class="lbl">Sytuacyjne — neutralne pytania o fakty, budują obraz sytuacji klienta</div></div>
  <div class="tile"><div class="big">P</div><div class="lbl">Problemowe — pytania o ukryte trudności, przełączają uwagę z rutyny na skanowanie problemu</div></div>
  <div class="tile"><div class="big">I</div><div class="lbl">Implikacyjne — pytania o konsekwencje zaniechania, klient sam dochodzi do skali problemu</div></div>
  <div class="tile"><div class="big">N</div><div class="lbl">Naprowadzające — pytania o wartość rozwiązania, klient sam zaczyna opisywać ulgę</div></div>
</div>

<h3>Jak to się przekłada na przebieg rozmowy — model treningowy</h3>
<p>Pytania <b>S i P</b> delikatnie podnoszą czujność klienta (w tym modelu opisywane jako „wzrost kortyzolu") — bez tego kroku klient nie poczuje żadnej motywacji do zmiany, zostaje w strefie komfortu. Pytania <b>I</b> to moment, w którym klient SAM, własnymi słowami, nazywa konsekwencje — to kluczowa różnica względem nacisku: <b>napięcie generuje klient, nie Ty</b>. Dopiero pytania <b>N</b> pytają o wartość rozwiązania — klient sam zaczyna opisywać, jak bardzo by mu ulżyło, zanim jeszcze cokolwiek zaproponujesz.</p>

<div class="psy"><b>Dlaczego to nie jest sprzeczne z Challenger Sale (moduł 2):</b> Pytania Implikacyjne to dokładnie to samo „konstruktywne napięcie", o którym mówi Take Control — z jedną kluczową różnicą: napięcie budujesz PYTANIAMI, nie stwierdzeniami czy presją czasową. Klient dochodzi do wniosku sam, odpowiadając na Twoje pytanie — Ty nigdy nie mówisz mu wprost „musi się Pan bać". To jest różnica między budowaniem wglądu a wywieraniem presji.</p></div>

<h3>Zamknięcie: dwa zupełnie różne rodzaje „nacisku"</h3>
<table class="tbl">
<tr><th></th><th>Toksyczne domykanie</th><th>Wspólny Plan Działania</th></tr>
<tr><td>Przykład zdania</td><td>„Bierze Pan czy nie?"</td><td>„Krok 1 dziś, krok 2 we wtorek — robimy tak?"</td></tr>
<tr><td>Co czuje klient</td><td>osaczenie, presja czasu</td><td>partnerstwo, konkretny plan</td></tr>
<tr><td>W tym modelu</td><td>toksyczny skok napięcia</td><td>uspokojenie, poczucie wspólnoty</td></tr>
<tr><td>Skutek długoterminowy</td><td>żal, brak poleceń</td><td>zaufanie, naturalne polecenia</td></tr>
</table>

<p>To jest dokładnie ta sama zasada co w module 2 (Challenger bez nacisku): <b>Take Control</b> zawsze oznacza konkretny, wspólnie ustalony następny krok — nigdy presję czasową typu „teraz albo nigdy". Etap zamknięcia opisywany w tym modelu jako budowanie zaufania i naturalny system poleceń (na bazie prac Michaela Mahera i Boba Burga) to w praktyce to samo „prowadzenie do decyzji we współpracy z klientem", które przewija się przez całą tę akademię.</p>

<div class="acc">
  <div class="acc-h" onclick="toggleAcc(this)">Ściągawka do tłumaczenia innym na warsztacie <span class="ico">+</span></div>
  <div class="acc-b"><p>„SPIN Selling to nie była technika lingwistyczna, to był pierwszy podręcznik prowadzenia rozmowy tak, żeby klient SAM doszedł do wniosku, że potrzebuje zmiany. Pytania sytuacyjne i problemowe delikatnie budzą czujność. Pytania implikacyjne pokazują konsekwencje — ale to klient je wypowiada, nie Ty. Pytania naprowadzające otwierają drzwi do ulgi. Zamknięcie przez wspólny plan buduje zaufanie, nie presję. Kto prowadzi rozmowę w tej kolejności, prowadzi ją etycznie i skutecznie naraz — to nie są sprzeczne cele."</p></div>
</div>

<div class="quiz">
  <div class="q">Który element sprawia, że pytania Implikacyjne NIE są formą nacisku, tylko elementem Take Control z Challenger Sale?</div>
  <button class="opt" data-ok="1" onclick="quizPick(this,true,'Dokładnie — to sedno tego modułu. Napięcie budowane pytaniem, na które klient sam odpowiada, to insight, nie presja.','')">To klient własnymi słowami nazywa konsekwencje, odpowiadając na pytanie — Ty nie stwierdzasz tego za niego</button>
  <button class="opt" data-ok="0" onclick="quizPick(this,false,'','To odwrotność mechanizmu — ton nie ma tu znaczenia decydującego, kluczowe jest KTO wypowiada konsekwencje: klient, nie sprzedawca.')">To, że mówisz to bardzo spokojnym, łagodnym tonem</button>
  <div class="fb"></div>
</div>
`;

CONTENT.scenariusz_rolnik = `
<img class="mod-icon reveal" src="https://media.base44.com/images/public/69cd0d788375531a373c98b1/99116576f_generated_image.png" alt="" loading="lazy">
<span class="eyebrow">Moduł 26 · Mechanizm głębszy — pełna symulacja</span>
<h2>Pełna symulacja: rolnik z Pokolenia X (magazyn energii)</h2>
<p class="lede">Janusz, ok. 50 lat, Pokolenie X, mieszanka typów D i C (twardy zysk i niezależność typu D, ostrożność wobec „nowinek" typu C) — profil zgodny z modułami 9 i 12 tej akademii. Poniżej pełna rozmowa pokazująca SPIN + kod kulturowy wsi w praktyce: zero żargonu korporacyjnego, proste, twarde słowa.</p>

<h3>Etap 1–2: Zagajenie i hak (Situation, wejście)</h3>
<div class="scene">
  <div class="scene-title">W terenie, przy gospodarstwie</div>
  <div class="bubble c"><span class="who">Doradca</span>Dzień dobry, panie Januszu. Krótka sprawa, bez owijania w bawełnę. Jeżdżę po gospodarstwach tutaj w okolicy i sprawdzam, jak rolnicy radzą sobie z tymi ciągłymi wyłączeniami prądu przez sieć. Nie przyjechałem nic wciskać ani podpisywać umów — chcę tylko na sucho przeliczyć, ile Pan traci, jak w środku obrządku czy dojenia odcinają zasilanie. Ma Pan 5 minut przy ciągniku?</div>
  <div class="bubble k"><span class="who">Janusz</span>No, odcinają dranie, ostatnio dwa razy w tygodniu transformator wywaliło! Ale te magazyny to droga zabawka, znowu naciąganie.</div>
</div>

<h3>Etap 3: Pytania Problemowe i Implikacyjne — rozbieranie na części</h3>
<div class="scene">
  <div class="scene-title">Klient sam dochodzi do skali problemu</div>
  <div class="bubble c"><span class="who">Doradca</span>Rozumiem, tanie to nie jest, i gdyby nie dawało zwrotu, sam bym odradził. Ale zapytam wprost: jak sieć odcina prąd na 3-4 godziny w środku pracy, to co się dzieje z urządzeniami? Jak akurat schładzalnik mleka albo wentylacja stanie w upał — ile Pan wtedy traci?</div>
  <div class="bubble k"><span class="who">Janusz</span>No stres, agregat trzeba odpalać, a ta cholera na ropę żre tyle, że głowa boli. Ostatnio mało brakowało, żeby partia mleka poszła do wylania.</div>
  <div class="bubble c"><span class="who">Doradca</span>Czyli jeśli sieć znowu siądzie w te żniwa, to pali Pan tysiące w agregacie, a w gorszym wypadku ryzykuje stratę całego udoju?</div>
  <div class="bubble k"><span class="who">Janusz</span>No tak to wygląda, nerwica człowieka bierze z tym prądem.</div>
  <div class="note">Konsekwencje wypowiada Janusz, nie doradca — to klucz różniący to od nacisku.</div>
</div>

<h3>Etap 4–5: Pytanie Naprowadzające i Wspólny Plan Działania</h3>
<div class="scene">
  <div class="scene-title">Ulga i konkretny plan</div>
  <div class="bubble c"><span class="who">Doradca</span>A gdybyśmy postawili magazyn energii przy budynkach — jak sieć siada, system w ułamku sekundy przejmuje zasilanie, agregat stoi cicho, chłodzenie idzie bez przerwy. Do tego ładuje się za darmo ze słońca w dzień. Ile spokoju i gotówki zostaje wtedy w kieszeni?</div>
  <div class="bubble k"><span class="who">Janusz</span>No wtedy miałbym tę sieć głęboko gdzieś. Ale to pewnie skomplikowane w obsłudze?</div>
  <div class="bubble c"><span class="who">Doradca</span>Sprawa jest prosta. Dziś podajemy sobie rękę, w przyszły wtorek moi ludzie montują pod pełne zabezpieczenie, Pan tylko patrzy jak prąd jest zawsze. Ja biorę papierologię na siebie. Zabezpieczamy gospodarstwo przed żniwami?</div>
  <div class="bubble k"><span class="who">Janusz</span>Dobra, jak papierami zajmujesz się ty, a sprzęt ma twardą gwarancję — piszmy to.</div>
</div>

<div class="card">
<p><b>Dlaczego to zadziałało:</b> zero słów typu „optymalizacja" czy „ROI" — słownictwo kręci się wokół udoju, ropy, zepsutego towaru, ciężkiej pracy. Domknięcie na zasadzie prostego, męskiego ustalenia i podania ręki, nie formalnego „podpisania umowy". To dokładnie Tailor z modułu 2 — ten sam mechanizm SPIN, inny kod językowy.</p>
</div>

<div class="quiz">
  <div class="q">Co sprawiło, że pytanie implikacyjne u Janusza nie zabrzmiało jak nacisk?</div>
  <button class="opt" data-ok="1" onclick="quizPick(this,true,'Dokładnie — Janusz sam nazwał konsekwencje (nerwica, straty), doradca tylko zadał pytanie o skutki.','')">To Janusz własnymi słowami opisał stres i straty — doradca tylko zapytał o konsekwencje</button>
  <button class="opt" data-ok="0" onclick="quizPick(this,false,'','Doradca w ogóle nie podnosił tonu ani nie epatował strachem wprost — cała siła pytania leży w tym, że odpowiada na nie klient.')">Doradca podniósł ton głosu, żeby wzbudzić w nim strach</button>
  <div class="fb"></div>
</div>
`;

CONTENT.scenariusz_manager = `
<img class="mod-icon reveal" src="https://media.base44.com/images/public/69cd0d788375531a373c98b1/99116576f_generated_image.png" alt="" loading="lazy">
<span class="eyebrow">Moduł 27 · Mechanizm głębszy — pełna symulacja</span>
<h2>Pełna symulacja: manager Millenialsi w korporacji (magazyn energii)</h2>
<p class="lede">Marek, 38 lat, Millenialsi, mieszanka typów S i C (bezpieczeństwo wizerunkowe i procedury typu C, potrzeba stabilności typu S) — profil zgodny z modułami 16 i 17 tej akademii. Ten sam produkt, zupełnie inny kod językowy: KPI, zarząd, procedury.</p>

<h3>Etap 1–2: Zagajenie i hak (Situation)</h3>
<div class="scene">
  <div class="scene-title">Rozmowa telefoniczna / wideorozmowa</div>
  <div class="bubble c"><span class="who">Doradca</span>Dzień dobry, panie Marku. Nawiązuję do danych z audytu efektywności energetycznej dla sektora logistycznego — od nowego kwartału taryfy dynamiczne uderzą w koszty utrzymania serwerowni i klimatyzacji. Managerowie operacyjni z Państwa branży obawiają się teraz przekroczenia budżetów. Organizujemy bezpłatne, 15-minutowe analizy profilu zużycia, bez zobowiązań — wtorek 10:00?</div>
  <div class="bubble k"><span class="who">Marek</span>Koszty energii to u nas gorący temat, prezes patrzy na każdy ułamek procenta. Ale mamy już fotowoltaikę, więc jesteśmy zabezpieczeni.</div>
</div>

<h3>Etap 3: Pytania Problemowe i Implikacyjne</h3>
<div class="scene">
  <div class="scene-title">Klient sam dochodzi do ryzyka wizerunkowego</div>
  <div class="bubble c"><span class="who">Doradca</span>Świetnie, że macie własną produkcję. Z symulacji wynika jednak, że w godzinach szczytu produkcji sieć obniża ceny odkupu, a wieczorem, gdy serwery i klimatyzacja pracują na maksimum, płacicie pełną stawkę. Co się stanie ze wskaźnikami KPI Pana działu, jeśli te dysproporcje odetną 20% planowanych oszczędności w tym roku?</div>
  <div class="bubble k"><span class="who">Marek</span>To byłaby katastrofa. Jeśli instalacja generuje prąd, którego nie potrafimy zoptymalizować finansowo, prezes obarczy mnie odpowiedzialnością za zły dobór technologii, a budżet jest sztywny.</div>
  <div class="bubble c"><span class="who">Doradca</span>Czyli bez bufora ryzykuje Pan sytuację, w której flagowy projekt energetyczny zamiast zysków przyniesie tłumaczenie się przed CFO z przestrzelonych wskaźników ROI?</div>
  <div class="bubble k"><span class="who">Marek</span>Dokładnie tak. Potrzebuję rozwiązania, które zamknie ten temat raz a dobrze.</div>
</div>

<h3>Etap 4–5: Pytanie Naprowadzające i Wspólny Plan Działania</h3>
<div class="scene">
  <div class="scene-title">Ulga w postaci gotowego raportu</div>
  <div class="bubble c"><span class="who">Doradca</span>A gdyby dedykowany magazyn energii automatycznie przechwytywał nadwyżki z fotowoltaiki w południe i uwalniał je dokładnie wtedy, gdy taryfa jest najdroższa? System działa autonomicznie, a Pan co miesiąc dostaje gotowy raport PDF z wyliczonymi oszczędnościami, który od razu może Pan przekazać do zarządu. Jak bardzo odciążyłoby to Pana z bieżącej kontroli tych kosztów?</div>
  <div class="bubble k"><span class="who">Marek</span>To brzmi idealnie — automatyczny raport to dokładnie podkładka, jakiej potrzebuję dla dyrektora finansowego. Jaka jest procedura wdrożenia?</div>
  <div class="bubble c"><span class="who">Doradca</span>W pełni bezpieczna. Mam dla Pana Wspólny Plan Działania w krokach, zgodny z procedurami ISO. Krok 1: zdalny audyt przyłączy bez zakłócania pracy serwerów. Krok 2: specyfikacja i wzór umowy dla działu prawnego. Krok 3: montaż w weekend. Przechodzimy do kroku pierwszego, żeby zdążyć przed nowym okresem rozliczeniowym?</div>
  <div class="bubble k"><span class="who">Marek</span>Świetnie ustrukturyzowane. Poproszę plan na maila wraz z umową dla prawników, zabezpieczam termin na audyt.</div>
</div>

<div class="card">
<p><b>Dlaczego to zadziałało:</b> słowa-klucze to „KPI", „zarząd", „CFO", „procedury ISO" — doradca sprzedał nie magazyn energii, tylko <b>bezpieczeństwo wizerunkowe i gotowy raport sukcesu dla prezesa</b>. To ten sam mechanizm co u Janusza, kompletnie inny słownik — bo inny kod kulturowy grupy.</p>
</div>

<div class="quiz">
  <div class="q">Co konkretnie „sprzedał" doradca Markowi, ponad sam magazyn energii?</div>
  <button class="opt" data-ok="1" onclick="quizPick(this,true,'Dokładnie — dla tego profilu bezpieczeństwo wizerunkowe i gotowy dowód sukcesu dla przełożonych są ważniejsze niż sama technologia.','')">Bezpieczeństwo wizerunkowe wobec zarządu i gotowy, automatyczny dowód sukcesu (raport)</button>
  <button class="opt" data-ok="0" onclick="quizPick(this,false,'','Cena w ogóle nie padła w tej rozmowie — to nie ona była głównym motywatorem decyzji Marka.')">Najniższą cenę na rynku</button>
  <div class="fb"></div>
</div>
`;

CONTENT.ciagalt_kulturowy = `
<img class="mod-icon reveal" src="https://media.base44.com/images/public/69cd0d788375531a373c98b1/99116576f_generated_image.png" alt="" loading="lazy">
<span class="eyebrow">Moduł 28 · Mechanizm głębszy</span>
<h2>Ściąga: dopasowanie języka do kodu kulturowego grupy</h2>
<p class="lede">Ten sam mechanizm SPIN + Take Control, dwa całkowicie różne słowniki. To ostatni element układanki — Tailor z Challenger Sale w praktyce nie kończy się na pokoleniu i typie DISC, sięga też do środowiska zawodowego i kulturowego rozmówcy.</p>

<table class="tbl">
<tr><th></th><th>Środowisko wiejskie / rzemieślnicze</th><th>Środowisko korporacyjne / biurowe</th></tr>
<tr><td>Słowa do UŻYCIA</td><td>zysk, spokój, gospodarstwo, ręka na zgodę, prosto</td><td>KPI, zarząd, procedura, raport, wskaźniki, ROI</td></tr>
<tr><td>Słowa do UNIKANIA</td><td>optymalizacja, wdrożenie, ISO, synergia</td><td>żargon potoczny, zbytnia poufałość</td></tr>
<tr><td>Dowód, który przekonuje</td><td>konkretna strata (ropa, zepsuty towar, czas)</td><td>gotowy dokument/raport do przekazania wyżej</td></tr>
<tr><td>Forma domknięcia</td><td>uścisk dłoni, prosta umowa ustna+podpis</td><td>formalny Wspólny Plan Działania, mail, procedura</td></tr>
</table>

<div class="psy"><b>Zasada ogólna:</b> zanim zaczniesz mówić, zapytaj się „jakim językiem ta osoba opisuje SWOJĄ pracę i swoje problemy" — i użyj dokładnie tego słownika, nie swojego korporacyjnego czy sprzedażowego. To najszybszy sposób, by insight (Teach) zabrzmiał jak zrozumienie, a nie jak prezentacja.</div>

<div class="card">
<p><b>Jak to połączyć z resztą akademii:</b> pokolenie i typ DISC mówią Ci CO i JAK, kod kulturowy środowiska mówi Ci JAKIM SŁOWNIKIEM. Janusz (moduł 26) to Pokolenie X × D/C w kodzie wiejskim — porównaj jego rozmowę z modułem 9 (X×D) i 12 (X×C). Marek (moduł 27) to Millenialsi × S/C w kodzie korporacyjnym — porównaj z modułami 16 (Y×S) i 17 (Y×C).</p>
<p>Poniżej (moduły 29–31) znajdziesz jeszcze trzy pełne symulacje — inne pokolenia, inne kanały kontaktu, inny kod kulturowy: Baby Boomer × I w terenie, Pokolenie X × S przez telefon i Pokolenie Z × I na czacie. Razem to 5 pełnych rozmów pokazujących ten sam mechanizm SPIN + Take Control w zupełnie różnych realiach.</p>
</div>

<div class="quiz">
  <div class="q">Doradca rozmawia z rolnikiem i mówi „wdrożymy synergiczne rozwiązanie zoptymalizowane pod Państwa procesy operacyjne". Co jest nie tak?</div>
  <button class="opt" data-ok="1" onclick="quizPick(this,true,'Dokładnie — to złamanie zasady Tailor: żargon korporacyjny w środowisku, które mówi prosto i konkretnie, brzmi obco i budzi nieufność zamiast zaufania.','')">Użyto korporacyjnego żargonu zamiast prostego, konkretnego języka pasującego do środowiska rozmówcy</button>
  <button class="opt" data-ok="0" onclick="quizPick(this,false,'','Poprawność językowa nie jest tu problemem — problemem jest niedopasowanie kodu kulturowego do rozmówcy.')">Zdanie jest po prostu gramatycznie niepoprawne</button>
  <div class="fb"></div>
</div>
`;

CONTENT.scenariusz_babyboomer_i = `
<img class="mod-icon reveal" src="https://media.base44.com/images/public/69cd0d788375531a373c98b1/99116576f_generated_image.png" alt="" loading="lazy">
<span class="eyebrow">Moduł 29 · Mechanizm głębszy — pełna symulacja</span>
<h2>Pełna symulacja: Baby Boomer × Wpływowy (instalacja PV, teren)</h2>
<p class="lede">Pani Krystyna, 66 lat, emerytowana pielęgniarka, typ I — profil zgodny z modułem 5 (Baby Boomers × I). Kontakt bezpośredni, drzwi otwarte, kawa na stole. SPIN w tym kodzie kulturowym brzmi zupełnie inaczej niż u Janusza czy Marka — więcej ciepła, mniej liczb na starcie.</p>

<h3>Etap 1–2: Zagajenie i hak (relacja przed sedno)</h3>
<div class="scene">
  <div class="scene-title">Próg domu, popołudnie</div>
  <div class="bubble c"><span class="who">Doradca</span>Dzień dobry pani! Ale ładnie tu pani ma te pelargonie na balkonie — sam próbuję je hodować i marnie mi idzie. A propos sąsiedztwa — słyszała pani, że pani Halina z tej ulicy już ma panele na dachu?</div>
  <div class="bubble k"><span class="who">Pani Krystyna</span>O, Halina? Nie wiedziałam! No proszę wejść, akurat mam kawę zaparzoną. A drogo to było?</div>
</div>

<h3>Etap 3: Pytania Problemowe i Implikacyjne — przez historię, nie liczby</h3>
<div class="scene">
  <div class="scene-title">Napięcie budowane opowieścią, nie danymi</div>
  <div class="bubble c"><span class="who">Doradca</span>Halina mówiła, że najbardziej denerwowały ją te rachunki zimą, jak trzeba było grzać cały dzień. A jak u pani teraz wygląda zima z rachunkami?</div>
  <div class="bubble k"><span class="who">Pani Krystyna</span>Ojej, nie mówmy o tym — w grudniu przyszedł rachunek i mąż o mało zawału nie dostał. A na emeryturze to nie ma z czego odkładać na takie niespodzianki.</div>
  <div class="bubble c"><span class="who">Doradca</span>To musi być stresujące, nie wiedzieć z miesiąca na miesiąc, ile przyjdzie zapłacić. Halina mówiła podobnie, zanim się zdecydowała.</div>
  <div class="bubble k"><span class="who">Pani Krystyna</span>No właśnie, człowiek się boi otwierać kopertę z rachunkiem, taki mamy wiek, żeby się teraz stresować.</div>
  <div class="note">Konsekwencje (stres, strach przed kopertą) wypowiada Pani Krystyna — doradca tylko prowadzi rozmowę pytaniami i odniesieniem do sąsiadki.</div>
</div>

<h3>Etap 4–5: Pytanie Naprowadzające i ciepłe domknięcie</h3>
<div class="scene">
  <div class="scene-title">Ulga podana jako wspólnota, nie transakcja</div>
  <div class="bubble c"><span class="who">Doradca</span>A gdyby ten strach przed kopertą po prostu zniknął? Halina mówiła, że teraz nawet nie sprawdza rachunku ze stresem, tylko z ciekawości, ile zaoszczędziła. Jak by to było, gdyby pani też tak mogła?</div>
  <div class="bubble k"><span class="who">Pani Krystyna</span>Ojej, to by było wspaniale, żeby się nie bać zimy. Ale mąż będzie musiał to zobaczyć, on się zna na takich sprawach.</div>
  <div class="bubble c"><span class="who">Doradca</span>Jak najbardziej, niech pan mąż też to zobaczy — może przyjadę w sobotę, kiedy oboje będziecie w domu, przyniosę te same papiery co Halina miała, i opowiem dokładnie jak to u niej wyglądało krok po kroku?</div>
  <div class="bubble k"><span class="who">Pani Krystyna</span>To bardzo dobry pomysł, zapraszam w sobotę na tę kawę, to se pogadamy z mężem spokojnie.</div>
</div>

<div class="card">
<p><b>Dlaczego to zadziałało:</b> zero liczb w pierwszych trzech wymianach zdań — most zaufania budowany przez sąsiedzkie odniesienie i wspólne emocje, dokładnie jak w module 5 (Take Control dla Baby Boomer × I to domknięcie osobiste, nie transakcyjne). Konkretny termin (sobota, z mężem) to Wspólny Plan Działania dopasowany do tego, że u Boomerów decyzja często wymaga rodziny.</p>
</div>

<div class="quiz">
  <div class="q">Dlaczego doradca nie podał żadnej konkretnej liczby złotych w pierwszej części rozmowy z Panią Krystyną?</div>
  <button class="opt" data-ok="1" onclick="quizPick(this,true,'Dokładnie — dla typu I w tym pokoleniu liczby na starcie zabiłyby budowaną właśnie relację. Konkret przychodzi dopiero po zbudowaniu zaufania, w sobotę z mężem.','')">Bo typ I najpierw potrzebuje relacji i zaufania — liczby na starcie zabrzmiałyby zbyt transakcyjnie</button>
  <button class="opt" data-ok="0" onclick="quizPick(this,false,'','To nie było przypadkowe pominięcie — to świadome dopasowanie kolejności do tego, jak typ I podejmuje decyzje.')">Doradca po prostu zapomniał podać liczby</button>
  <div class="fb"></div>
</div>
`;

CONTENT.scenariusz_x_s = `
<img class="mod-icon reveal" src="https://media.base44.com/images/public/69cd0d788375531a373c98b1/99116576f_generated_image.png" alt="" loading="lazy">
<span class="eyebrow">Moduł 30 · Mechanizm głębszy — pełna symulacja</span>
<h2>Pełna symulacja: Pokolenie X × Stały (klimatyzacja, telefon)</h2>
<p class="lede">Pan Tomasz, 44 lata, mechanik samochodowy, typ S — profil zgodny z modułem 11 (Pokolenie X × S). Kontakt telefoniczny, nieufność wobec „kolejnej oferty" bardzo wysoka. SPIN musi tu iść wolniej niż gdziekolwiek indziej w tej akademii.</p>

<h3>Etap 1–2: Zagajenie z góry rozbrajające nieufność</h3>
<div class="scene">
  <div class="scene-title">Rozmowa telefoniczna</div>
  <div class="bubble c"><span class="who">Doradca</span>Dzień dobry, panie Tomaszu. Wiem, że pewnie dostaje pan takich telefonów sporo, więc powiem wprost — nie proszę o żadną decyzję dzisiaj, chcę tylko zapytać, jak u pana wygląda temat chłodzenia w warsztacie latem?</div>
  <div class="bubble k"><span class="who">Pan Tomasz</span>No dostaję te telefony co drugi dzień, wszyscy chcą coś wcisnąć. Latem to jest gorąco jak w piekarniku, ale jakoś żyjemy.</div>
</div>

<h3>Etap 3: Pytania Problemowe i Implikacyjne — bardzo powoli, z ciszą</h3>
<div class="scene">
  <div class="scene-title">Klient sam, bez pośpiechu, dochodzi do sedna</div>
  <div class="bubble c"><span class="who">Doradca</span>Rozumiem, że jakoś się żyje. A jak to wygląda z pracownikami w te najgorętsze dni — nie ma jakichś przestojów albo że komuś słabo się robi przy silnikach?</div>
  <div class="bubble k"><span class="who">Pan Tomasz</span>No bywa, że chłopaki muszą robić przerwy co godzinę, bo się gotują. Raz nawet jeden zasłabł, musieliśmy go do domu odsyłać.</div>
  <div class="bubble c"><span class="who">Doradca</span>To musi wpływać na to, ile zleceń dziennie realnie da się zrobić w lipcu-sierpniu?</div>
  <div class="bubble k"><span class="who">Pan Tomasz</span>No pewnie, latem robimy z 30% mniej niż zimą, ludzie po prostu nie wyrabiają w tym upale.</div>
  <div class="note">Doradca nie przyspiesza mimo pauz w rozmowie — to kluczowe dla typu S.</div>
</div>

<h3>Etap 4–5: Pytanie Naprowadzające i bardzo ostrożne domknięcie</h3>
<div class="scene">
  <div class="scene-title">Zero presji, mały odwracalny krok</div>
  <div class="bubble c"><span class="who">Doradca</span>A gdyby dało się utrzymać warsztat w rozsądnej temperaturze przez cały sezon, bez tych przestojów — ile by to mogło zmienić w liczbie zleceń miesięcznie?</div>
  <div class="bubble k"><span class="who">Pan Tomasz</span>To by było spore, ale ja się na tych ofertach nie znam, i nie chcę żadnego zobowiązania na dziś.</div>
  <div class="bubble c"><span class="who">Doradca</span>Żadnego zobowiązania nie ma i na dziś nic nie musi pan decydować. Zostawię panu tylko numer i link do naszej strony — proszę spokojnie sprawdzić opinie, zapytać kogoś kto już to ma, i jak pan będzie gotów, sam pan zadzwoni albo ja zadzwonię za dwa tygodnie tylko zapytać jak leci.</div>
  <div class="bubble k"><span class="who">Pan Tomasz</span>To mi pasuje, niech pan tak zrobi, za dwa tygodnie może pan zadzwonić, na razie se to poukładam w głowie.</div>
</div>

<div class="card">
<p><b>Dlaczego to zadziałało:</b> doradca explicite zdjął presję czasową już na starcie rozmowy ("nie proszę o żadną decyzję dzisiaj") — dokładnie zasada z modułu 11. Wspólny Plan Działania tutaj to nie termin montażu, tylko termin KOLEJNEJ ROZMOWY za dwa tygodnie — mały, bezpieczny krok zamiast dużej decyzji.</p>
</div>

<div class="quiz">
  <div class="q">Dlaczego doradca zaproponował telefon za dwa tygodnie zamiast próbować umówić montaż od razu?</div>
  <button class="opt" data-ok="1" onclick="quizPick(this,true,'Dokładnie — dla typu S presja na decyzję w pierwszej rozmowie niemal zawsze kończy się odmową, a mały bezpieczny krok buduje zaufanie do kolejnej rozmowy.','')">Bo typ S potrzebuje czasu i poczucia braku presji — mały krok buduje zaufanie do kolejnej rozmowy</button>
  <button class="opt" data-ok="0" onclick="quizPick(this,false,'','To nie był brak umiejętności zamykania sprzedaży — to świadome dopasowanie tempa do typu klienta.')">Bo doradca nie potrafił dobrze domknąć rozmowy</button>
  <div class="fb"></div>
</div>
`;

CONTENT.scenariusz_z_i = `
<img class="mod-icon reveal" src="https://media.base44.com/images/public/69cd0d788375531a373c98b1/99116576f_generated_image.png" alt="" loading="lazy">
<span class="eyebrow">Moduł 31 · Mechanizm głębszy — pełna symulacja</span>
<h2>Pełna symulacja: Pokolenie Z × Wpływowy (panele PV, czat/Instagram)</h2>
<p class="lede">Zosia, 21 lat, pierwsza mieszkanka własnego M3, typ I — profil zgodny z modułem 20 (Pokolenie Z × I). Cały kontakt przez wiadomości, zaczęty od krótkiego wideo z realizacji. SPIN skondensowany do kilku krótkich wymian, zero telefonu.</p>

<h3>Etap 1–2: Zagajenie przez wideo, hak w 2 zdaniach</h3>
<div class="scene">
  <div class="scene-title">Wiadomości na Instagramie, wieczorem</div>
  <div class="bubble c"><span class="who">Doradca</span>Hej! Widziałaś to wideo co wrzuciliśmy z montażu u Kasi w Twoim wieku? Miała dokładnie taki sam blok jak Ty 😊 Chcesz zobaczyć ile jej to realnie oszczędza miesięcznie?</div>
  <div class="bubble k"><span class="who">Zosia</span>ooo widziałam to na stories! no pokaż bo ja płacę kosmos za prąd w tym mieszkaniu, ledwo starcza</div>
</div>

<h3>Etap 3: Pytania Problemowe i Implikacyjne — szybko, konkretnie</h3>
<div class="scene">
  <div class="scene-title">Napięcie budowane w 2 wymianach, nie 5</div>
  <div class="bubble c"><span class="who">Doradca</span>ile średnio wychodzi Ci miesięcznie? i co robisz jak przychodzi wyższy rachunek niż myślałaś?</div>
  <div class="bubble k"><span class="who">Zosia</span>z 280 zł czasem, i szczerze to mnie wtedy szlag trafia bo to jakby połowa mojego budżetu na jedzenie ucieka</div>
  <div class="bubble c"><span class="who">Doradca</span>czyli realnie zamiast na siebie, te pieniądze co miesiąc lecą do dostawcy prądu?</div>
  <div class="bubble k"><span class="who">Zosia</span>dokładnie tak to wygląda jak to ujmujesz, boli</div>
  <div class="note">Zosia sama nazywa problem (utracony budżet) — doradca tylko zadaje krótkie pytania.</div>
</div>

<h3>Etap 4–5: Pytanie Naprowadzające i lekkie domknięcie</h3>
<div class="scene">
  <div class="scene-title">Ulga podana lekko, bez presji</div>
  <div class="bubble c"><span class="who">Doradca</span>a gdyby te 280 zł spadło do jakichś 40-50 zł, co byś z tą różnicą zrobiła? 😄</div>
  <div class="bubble k"><span class="who">Zosia</span>ja bym chyba płakała ze szczęścia haha, ale czy to nie jest jakiś przekręt, tak za dobrze brzmi</div>
  <div class="bubble c"><span class="who">Doradca</span>rozumiem czujność, sprawdź sama — tu masz link do oficjalnej strony programu, wszystko się zgadza z tym co piszę. jak chcesz to wyślę Ci krótką wycenę dla Twojego metrażu, zero zobowiązań</div>
  <div class="bubble k"><span class="who">Zosia</span>okej sprawdziłam, wygląda spoko — wyślij tę wycenę, obczaję i się odezwę</div>
</div>

<div class="card">
<p><b>Dlaczego to zadziałało:</b> cała rozmowa trwała kilka minut, w naturalnym, swobodnym języku — zero formalności, zero telefonu. Weryfikacja (link do oficjalnej strony) odpowiada na czujność typową dla Pokolenia Z (moduł 21), a lekkie domknięcie „wyślę wycenę, zero zobowiązań" to Wspólny Plan Działania bez cienia presji, dopasowany do tempa czatu.</p>
</div>

<div class="quiz">
  <div class="q">Dlaczego doradca od razu podał link do oficjalnego źródła, gdy Zosia spytała czy to nie przekręt?</div>
  <button class="opt" data-ok="1" onclick="quizPick(this,true,'Dokładnie — Pokolenie Z ufa możliwości samodzielnej weryfikacji bardziej niż jakimkolwiek zapewnieniom słownym, zwłaszcza w kontakcie czysto cyfrowym.','')">Bo to pokolenie ufa możliwości samodzielnej weryfikacji bardziej niż słownym zapewnieniom</button>
  <button class="opt" data-ok="0" onclick="quizPick(this,false,'','To nie było wymijanie pytania — to celowe danie jej narzędzia do zbudowania zaufania samodzielnie, co dla tego pokolenia działa najlepiej.')">Żeby uniknąć odpowiedzi wprost na jej pytanie</button>
  <div class="fb"></div>
</div>
`;
